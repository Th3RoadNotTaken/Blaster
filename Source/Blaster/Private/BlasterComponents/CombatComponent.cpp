...
void UCombatComponent::SetHUDCrosshairs(float DeltaTime)
{
	...
	if (Controller)
	{
		...
		if (HUD)
		{
			...
			FVector2D VelocityMultiplierRangeWhenCrouched(0.f, 0.5f);
			FVector Velocity = Character->GetVelocity();
			Velocity.Z = 0.f;
			// Mapping character velocity in the range of [0,1] while walking and in the range [0,0.5] when crouched
			if (Character->GetCharacterMovement()->IsCrouching())
			{
				CrosshairVelocityFactor = FMath::GetMappedRangeValueClamped(CrouchSpeedRange, VelocityMultiplierRangeWhenCrouched, Velocity.Size());
			}
			else
			{
			...
			if (bAimingAtPlayer)
			{
				CrosshairAtPlayerFactor = FMath::FInterpTo(CrosshairAtPlayerFactor, -0.3f, DeltaTime, 30.f);
			}
			else
			{
				CrosshairAtPlayerFactor = FMath::FInterpTo(CrosshairAtPlayerFactor, 0.f, DeltaTime, 30.f);
			}
...
void UCombatComponent::SetAiming(bool bIsAiming)
{
	...
	We don't need to check if a client is calling this function since an RPC called from a client runs on the server and also an RPC called from the server
...
bool UCombatComponent::CanFire()
{
	if (EquippedWeapon == nullptr)
		return false;
	// Shotgun fire during reload condition
